...
var internalPrepCmd = &cobra.Command{
	Use:    "_prep",
...
func init() {
	rootCmd.AddCommand(internalPrepCmd)

	internalPrepCmd.Flags().StringVarP(&internalPrepFlags.UID, "uid", "u", "", "The UID of the external user.")
	internalPrepCmd.Flags().StringVarP(&internalPrepFlags.GID, "gid", "g", "", "The GID of the external user's group.")
	internalPrepCmd.Flags().StringVarP(&internalPrepFlags.HgCachePath, "hg-cache-path", "c", "", "The path to hg cache.")
	internalPrepCmd.Flags().StringVarP(&internalPrepFlags.HostIP, "host-ip", "i", "", "The ip address of the host.  This will be added to /etc/hosts as 'host'")
	internalPrepCmd.Flags().StringVarP(&internalPrepFlags.CacheKeyURL, "license-key-url", "k", "", "Download the cache.key file from the provided location rather than the default Statler URL")
}
...
func prep(_ *cobra.Command, _ []string) {
	...
	if internalPrepFlags.UID != "" && internalPrepFlags.GID != "" {
		...
		cmd("usermod", "-u", internalPrepFlags.UID, cacheusrName)
		cmd("groupmod", "-g", internalPrepFlags.GID, cacheusrName)

		cmd("find", "/", "-user", cacheusrUID, "-not", "-path", "/proc/*", "-exec", "chown", "-h", internalPrepFlags.UID, "{}", ";")
		cmd("find", "/", "-group", cacheusrGID, "-not", "-path", "/proc/*", "-exec", "chgrp", "-h", internalPrepFlags.GID, "{}", ";")
	...
	updateCacheKey(internalPrepFlags.CacheKeyURL)

	if internalPrepFlags.HgCachePath != "" {
		css("%SYS", cacheImport(internalPrepFlags.HgCachePath))
...
func addSSHKey() {
	...
	ioutil.WriteFile("/root/.ssh/id_rsa", []byte(iscenv.SSHKey), 0600)
	fmt.Println("\tSuccess!")
}
